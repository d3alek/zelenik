...
def handle_update(db, thing, state, value):
    ...
    try:
        ...
        return content_type, ('Could not parse json value %s. %s %s' % (state, thing, value))
    ...
    if state == 'desired': 
        ...
        return content_type, ('Not allowed to change state %s. %s %s' % (state, thing, value))

    back_url = ('/db/' + thing)
    return content_type, (REDIRECT % (back_url, 'Success.', back_url))
